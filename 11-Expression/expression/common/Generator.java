...
public class Generator<C> {
    ...
    public <E> void testRandom(
        ...
        testRandom(counter, variables, consumer, 1, 100, 100 / d,
                (vars, depth) -> generateFullDepth(vars, Math.min(depth, 3)));
    ...
    private <D, E> void testRandom(
            final TestCounter counter,
            final ExpressionKind.Variables<E> variables,
            final Consumer<Expr<D, E>> consumer,
            final int seq,
            final int levels,
            final int perLevel,
            final BiFunction<List<Node<D>>, Integer, Node<D>> generator) {
        counter.scope("Random tests #" + seq, () -> {
            ...
            for (int level = 0; level < levels; level++) {
                for (int j = 0; j < perLevel; j++) {
                    ...
                    consumer.accept(
                            Expr.of(generator.apply(Functional.map(vars, v -> Node.op(v.first())), level), vars));
                }
            }
